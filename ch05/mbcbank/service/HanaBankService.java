...
public class HanaBankService { // 서비스 계층은 CRUD 메서드 위주로 생성한다.
	...
	public void menu(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank, AccountDTO[] woriBank,
		...
		while (subRun) {
			System.out.println("------------------");
			System.out.println("하나은행 계좌 관리 메서드입니다.");
			System.out.println("1. 계좌 생성");
			System.out.println("2. 계좌 목록(은행원용)");
			System.out.println("3. 예금");
			System.out.println("4. 출금");
			System.out.println("5. 이체");
			System.out.println("6. 계좌 잔액 확인");
			...
			case "5":
				...
				transfer(inputInt, inputStr, hanaBank, woriBank, nhBank);
				break;

			case "6":
				System.out.println("===계좌 잔액 확인 메서드로 진입합니다.===");
				breakdown(inputInt, inputStr, hanaBank);
				break;
	...
	private void breakdown(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank) {
		// 내 하나은행 계좌의 입출금 내역을 본다.

		System.out.println("=======계좌 잔액 확인=======");

		System.out.print("내 계좌번호: ");
		String ano = inputStr.next();

		AccountDTO inputAccount = findAccount(ano, hanaBank); // 하나 은행
		// findAccountMe 메서드는 배열에서 객체를 찾아주고 객체를 리턴해준다. (반복 코드 배제용)
		...
		inputAccount.setBalance(inputAccount.getBalance());
	...
	private void transfer(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank, AccountDTO[] woriBank,
			AccountDTO[] nhBank) {
		...
		AccountDTO inputAccount2 = findAccount2(transferAno, hanaBank, woriBank, nhBank); // 모든 은행
		// findAccount2 메서드는 배열에서 객체를 찾아주고 객체를 리턴해준다. (반복 코드 배제용)
	...
	private AccountDTO findAccount2(String transferAno, AccountDTO[] hanaBank, AccountDTO[] woriBank,
		 ...
		 * 
		 * if (nhBank[i] != null) { // 하나은행 배열이 null이 아님
		 * 
		 * String dbAno = woriBank[i].getAno(); // 배열에 있는 계좌번호를 가져와 dbAno 변수에 넣음
		 * 
		 * if (dbAno.equals(transferAno)) { // 키보드로 입력된 계좌번호와 dbAno와 같은 값을 찾는다.
		 * 
		 * account = nhBank[i]; // 빈 객체에 찾은 객체를 넣는다. break;
		 * 
		 * } // 계좌번호가 같은지 판단 종료
		 * 
		 * } // 빈 객체가 아닌지 판단 종료
	...
	private void accountLists(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank) {
		...
		for (int i = 0; i < hanaBank.length; i++) {
			...
			AccountDTO accountlist = hanaBank[i]; // hanaBank[i] 가져온다
	...
	private void creatAccount(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank) {
